...
def wait_for_service(url, name, timeout=180):
    ...
    while time.time() - start_time < timeout:
        try:
            ...
            logger.debug("Still waiting for %s. Error: %s", name, e)
        time.sleep(2)
...
def run_inference(url, model, prompt, name):
    ...
    logger.info("   [%s] Sending prompt to %s: '%s'",
                name, url, payload['prompt'])
    try:
        ...
        logger.info("   [%s] Response: '%s'", name, text.strip())
        return text
    except Exception as e:
...
def test_dynamic_allocation():
    ...
    with open("controller_dynamic.log", "w") as log_file:
        ...
        logger.info("Controller started with PID %s", controller_process.pid)
        ...
        try:
            ...
            logger.info("Engine 1 (OPT) created at %s (Port: %s)",
                        url_opt, port_opt)
            ...
            logger.info("Engine 2 (Qwen #1) created at %s (Port: %s)",
                        url_qwen1, port_qwen1)
            ...
            logger.info("Engine 3 (Qwen #2) created at %s (Port: %s)",
                        url_qwen2, port_qwen2)

            if port_qwen2 == port_opt:
                logger.info("SUCCESS: Port recycled!")
            else:
                logger.info(
                    "NOTICE: New port assigned (Expected behavior in simple allocator). Old: %s, New: %s", port_opt, port_qwen2)
