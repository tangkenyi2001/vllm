...
from vllm.worker_controller.tests.utils.benchmark_utils import (
    ...
    compute_standard_flow_timing,
    compute_worker_controller_flow_timing,
    ...
    print_engine_status,
    print_flow_event_offsets,
    print_flow_stage_report,
...
def measure_worker_controller_cold_start(
    ...
    wc_flow_stages = compute_worker_controller_flow_timing(
        create_time_s=create_time,
        api_ready_time_s=api_ready_time,
        first_inference_time_s=first_inference_time,
        create_timings=create_timings,
        model_load_summary=model_load_summary,
        startup_timing=startup_timing,
    )
    print_flow_stage_report("Worker Controller", wc_flow_stages)
    ...
    if wc_flow_stages:
        result["wc_flow_stages"] = wc_flow_stages
...
def measure_standard_vllm_cold_start(
    ...
    std_flow_stages, std_event_offsets = compute_standard_flow_timing(
        log_lines=std_log_lines,
        spawn_call_time_s=spawn_end - spawn_begin,
        server_ready_time_s=server_ready_time,
    )
    print_flow_stage_report("Standard vLLM", std_flow_stages)
    print_flow_event_offsets("Standard vLLM", std_event_offsets)
    ...
    result["std_flow_stages"] = std_flow_stages
    result["std_event_offsets"] = std_event_offsets
    return result
...
def main():
    ...
    for model_index, model_info in enumerate(MODELS):
        ...
        for run_idx in range(1, RUNS_PER_MODEL + 1):
            ...
            print(
                f"\n[Standard vLLM] run {run_idx}/{RUNS_PER_MODEL} "
                f"for {model_label} (port {port})"
            )
